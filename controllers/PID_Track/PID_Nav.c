...
vel_t PID_Nav_Point_Tracker(pid_nav_t *pid_nav, fzy_pid_t *fzy_ptr1, fzy_pid_t *fzy_ptr2, path_t *path)
{
    ...
    if (pid_nav->nav_state == NAV_TRACK_FINAL)
    {
        nav_vel.vx = fuzzypid_cal(fzy_ptr1,Get_SPVS_X_Vel_mm(),pid_nav->x_set, Get_GPS_X_mm());
        nav_vel.vy = fuzzypid_cal(fzy_ptr2,Get_SPVS_Y_Vel_mm(),pid_nav->y_set, Get_GPS_Y_mm());
    }
